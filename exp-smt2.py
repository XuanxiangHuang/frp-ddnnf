...
# -*- coding:utf-8 -*-
#
#   Reproduce experiments (two-step method)
import csv
import resource
import sys
...
def ddnnf_guess_one_axp(data_name, dataset, ddnnf_file, feat_map, insts_seed_file, feats_file):
    ...
    for idx, line in enumerate(lines[:d_len]):
        ...
        smt_axp = []
        smt_weakaxp, n_atoms, n_fmls, failed, time_i = guess.two_step(pred, f_id)
        if smt_weakaxp and not failed:
            print("Answer Yes")
            fix = [False] * guess.nf
            for ii in smt_weakaxp:
                fix[ii] = True
            smt_axp = extor.find_axp(fix)
            smt_axps.append(smt_axp)
            assert f_id in smt_axp
            print(f"AXp: {smt_axp}")
        elif not failed:
            print('======== no AXp exists ========')
        else:
    ...
    exp_results += f"{ceil(len(smt_axps) / d_len * 100):.0f}({fail_count}) & "
    ...
    with open('results/smt/ddnnf_fm2.txt', 'a') as f:
        f.write(exp_results)
